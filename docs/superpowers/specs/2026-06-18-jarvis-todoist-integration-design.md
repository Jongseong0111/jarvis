...
- **API:** Todoist 통합 API(`https://api.todoist.com/api/v1`)를 대상으로 한다. (구 REST v2 `…/rest/v2`는 deprecated — 사용자 확인됨 2026-06-19.) **v1 실측 검증 완료(2026-06-19):**
  - 할일 목록/필터: **필터는 별도 엔드포인트** `GET /tasks/filter?query=<필터>`(예: `today | overdue`). 필터 없는 전체 목록은 `GET /tasks`. ⚠️ `GET /tasks?filter=`는 무시됨(v2 잔재).
  - **응답은 페이지네이션 래퍼** `{"results":[...],"next_cursor":...}` — v2의 bare array와 다름. (개인 규모라 첫 페이지만 읽고 next_cursor 미순회.)
  - task 객체: `id`,`content`,`project_id`,`due{string,date,...}`(둘 다 존재, `string`="매일 오후9시" 등),`checked`. **`url` 필드 없음**(표시에 미사용이라 무해).
  - 완료: `POST /tasks/{id}/close`. 추가: `POST /tasks`(content/due_string/project_id). 수정: `POST /tasks/{id}`. 삭제: `DELETE /tasks/{id}`.
