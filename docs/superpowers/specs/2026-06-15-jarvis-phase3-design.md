...
8. Notion **대시보드 페이지** 셋업 가이드(구역별 그룹 뷰 + 검색/필터 뷰)

> 핵심: 서버는 데이터를 깔끔히 채우기만 하고, "예쁘게 보이는 페이지"(구역별 그룹·검색)는
> Notion 의 rollup + 뷰로 표현한다. 정교한 뷰/레이아웃은 Notion API 로 못 만들므로 셋업 가이드로 일회 구성.
...
| 메모 | rich_text | 선택 |
| 구역 | rollup → 현재위치.구역 | **Notion 속성(코드 0)**. 대시보드 구역별 그룹용 |

> 구역 rollup 은 현재위치 relation 을 따라 Location 의 구역을 자동으로 끌어온다. 서버는 현재위치만
> 채우면 되고, 대시보드는 이 rollup 으로 거실/베란다/창고… 그룹을 만든다.
...
6. Items 에 **구역 rollup** 속성 추가(현재위치 → Location.구역)
7. **대시보드 페이지** 생성(일회성): 빈 페이지에 Items linked database 를 2개 뷰로 임베드
   - **구역별 보기**: 보드(또는 그룹) 뷰, 그룹 기준 = 구역 rollup → "거실 > 트롤리에 뭐 있나" 한눈에
   - **검색 보기**: 테이블 뷰 + 이름/구역/카테고리 필터 → "손톱깎이" 바로 검색
   - (사진 갤러리 뷰는 이미지 속성 추가 시 향후)
...
- **Notion 대시보드**: 위에서 추가한 "체온계"가 구역별 보기에서 거실 그룹(트롤리) 아래 보임 + 검색 뷰에서 이름으로 바로 찾힘
