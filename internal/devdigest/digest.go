...
// domains 는 공부 주제 도메인 목록이다. 미지정 시 코드가 이 중 하나를 무작위로 고른다.
var domains = []string{
	"언어", "웹·백엔드", "데이터베이스", "인프라",
	"데이터", "운영체제", "네트워크",
...
// topicTemp 는 공부 주제 생성 온도다. 같은 도메인이라도 세부 주제가 매번 달라지도록 0 보다 크게 둔다.
const topicTemp float32 = 1.0

// chooseDomain 은 요청 도메인이 있으면 그대로, 없으면 randIntn 으로 domains 중 하나를 고른다.
// 온도 0 이던 시절 LLM 이 매번 같은 도메인을 고르던 문제를 코드 레벨 무작위 선택으로 대체한다.
func chooseDomain(requested string, randIntn func(int) int) string {
	if strings.TrimSpace(requested) != "" {
		return requested
	}
	return domains[randIntn(len(domains))]
}
...
type GeminiGenerator struct {
	...
	randIntn func(int) int // 도메인 무작위 선택(테스트에서 주입)
}

// NewGenerator 는 GeminiGenerator 를 생성한다.
func NewGenerator(client *gemini.Client) *GeminiGenerator {
	return &GeminiGenerator{client: client, randIntn: rand.IntN}
}
...
func (g *GeminiGenerator) Generate(ctx context.Context, items []NewsItem) (DigestResult, error) {
	...
	domain := chooseDomain("", g.randIntn)
	raw, err := g.client.GenerateTextTemp(ctx, systemPrompt, buildPrompt(items, len(geek) > 0, domain), topicTemp)
	if err != nil {
...
// requestedDomain 이 비면 코드가 domains 중 하나를 무작위로 골라(매번 같은 도메인 방지),
...
func (g *GeminiGenerator) GenerateTopics(ctx context.Context, requestedDomain string) (TopicResult, error) {
	ctx = usage.WithFeature(ctx, "digest")
	domain := chooseDomain(requestedDomain, g.randIntn)
	raw, err := g.client.GenerateTextTemp(ctx, systemPrompt, buildTopicPrompt(domain), topicTemp)
	if err != nil {
...
// buildTopicPrompt 는 공부 주제 전용 프롬프트를 만든다(도메인은 호출 측에서 이미 결정됨).
func buildTopicPrompt(domain string) string {
	var sb strings.Builder
	sb.WriteString("개발 공부 주제를 생성하라.\n")
	sb.WriteString("- 도메인/주제: \"" + domain + "\" 에 대해 생성하라(더 구체적인 세부 주제로 깊게 들어가도 좋다).\n")
	sb.WriteString("- domain 필드에는 위 주제의 큰 분류명을 넣어라.\n")
	sb.WriteString("- 계층형 주제 3-5개: \"도메인 → 중분류 → 구체 개념\" 형식. 매번 다른 중분류·개념으로 다양하게 구성하라.\n\n")
...
func buildPrompt(items []NewsItem, hasGeekNews bool, domain string) string {
	var sb strings.Builder
	...
	sb.WriteString("   - 도메인: \"" + domain + "\" (이 도메인 안에서 생성. domain 필드에 이 분류명을 넣어라)\n")
	sb.WriteString("   - 계층형 주제 3-5개: \"도메인 → 중분류 → 구체 개념\" 형식. 매번 다른 중분류·개념으로 다양하게 구성하라.\n\n")
